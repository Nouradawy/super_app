...
class _OtpScreenState extends State<OtpScreen> {
  ...
  void initState() {
    ...
    final initEmail =
        (widget.email ?? supabase.auth.currentUser?.email ?? '').trim();
    ...
    try {
      ...
      if (widget.isProfile == false) {
        ...
        if (user != null) {
          ...
          if (roleId != null) {
            authCubit.updateRole(Roles.values[roleId - 1]);
          }
        }
  ...
  Widget _buildCompactEmailField() {
    ...
    final value =
        _emailController.text.trim().isEmpty ? hint : _emailController.text.trim();
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      TextButton(
                        onPressed: () {
                          if (_resending || _secondsLeft > 0 || _emailController.text.isEmpty) {
                            return;
                          }
                          _resend();
                        },
